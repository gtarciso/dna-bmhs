...
} thread_control; thread_control tc;

int dispatcher(int* found) {}
int worker() {}
// ### END THREAD UTILS
...
int main(int argc, char *argv[]) {
	...
	while (get_next_query_descripton(desc_query)) {
		...
		while (get_next_base_description(desc_dna)) {
			...
			time_computation += (end_computation.tv_sec - start_computation.tv_sec)
                * 1000000 + end_computation.tv_usec - start_computation.tv_usec;
